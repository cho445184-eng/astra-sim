...
- **Git submodules must be initialized**: All source code for external components (chakra, fmt, spdlog, analytical backends, etc.) lives in git submodules. `git submodule update --init --recursive` is required before any build. However, the chakra submodule commit (`450b2187...`) was force-pushed away from the fork `cho445184-eng/chakra.git`. The update script works around this by cloning the fork's HEAD and creating the `feeder_v3/` compatibility layer.
- **Chakra feeder_v3 compatibility layer**: The ASTRA-sim code expects `extern/graph_frontend/chakra/src/feeder_v3/` with a `Chakra::FeederV3` namespace (includes `DependancyResolver`, template-based accessors, `has_attr`/`get_attr` methods). The upstream/fork chakra only has `src/feeder/` in the `Chakra::` namespace. The update script creates this compatibility layer automatically. If you see build errors about missing `feeder_v3` or `FeederV3::`, the layer needs to be regenerated — run the update script or rebuild from scratch.

...
- **Bundled protobuf/Abseil/protoc (required)**: `extern/helper/abseil-cpp` and `extern/helper/protobuf` are always built from submodules; no system `protoc`, `libprotobuf`, or Abseil packages. Executables may need to link `AstraSimProtobufDeps` (static link propagation is incomplete).
...
- **ns-3 build uses bundled libbgp, no OpenMPI for compile**: BGP/RDMA routing uses [Nat-Lab/libbgp](https://github.com/Nat-Lab/libbgp) (pinned 0.6.3) vendored at `extern/network_backend/ns-3/src/bgp/libbgp` and built by `src/bgp/CMakeLists.txt`. `./build/astra_ns3/build.sh` configures ns-3 with `--disable-mpi` (AstraSimNetwork does not need system OpenMPI). The ns-3 build takes ~2 minutes.
